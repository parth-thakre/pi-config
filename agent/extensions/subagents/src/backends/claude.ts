...
/** Claude Code's project-directory escaping, including Windows separators. */
export function claudeProjectDirectory(cwd: string) {
  return cwd.replace(/[\\/:.]/g, "-");
}

function sessionFilePath(cwd: string, sessionId: string) {
  const projectDirectory = claudeProjectDirectory(cwd);
  return path.join(
    os.homedir(),
    ".claude",
    "projects",

...
export interface AgentRecord {
  ...
  label: string;
  phase?: string;
  /** Only explicit read-only agents may overlap. */
  readOnly?: boolean;
  state: AgentState;
  model?: string;
...
export interface WorkflowDetails {
  runId: string;
  /** Pi session/process that launched this run. */
  sessionId?: string;
  ownerPid?: number;
  name?: string;
  description?: string;
  ...
  startedAt: number;
  /** Canonical repository/worktree identity, persisted before any child starts. */
  repoRoot?: string;
  commonGitDir?: string;
  baseRef?: string;
  baseCommit?: string;

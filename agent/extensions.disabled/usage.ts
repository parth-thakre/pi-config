...
8. For each Codex CLI token_count event with last_token_usage, add:
   ...
   - total tokens from last_token_usage.total_tokens if present, otherwise input + output + cached input_read
...
13. Convert all computed USD costs to INR before presenting. Fetch the current USD→INR rate from a reliable source via shell when possible; if unavailable, use a clearly noted fallback rate of 1 USD = 83 INR. Do not show USD as the main price.
14. Present the result as concise Markdown:
   - One section per window: Last 1 day, Last 7 days, Last 30 days, Last 90 days
   - A table with columns: Source, Model, Messages/Turns, Input, Output, Cached In, Total Tokens, Price (INR)
   - A total row at the bottom of each table
   - Format token counts with commas and INR with the ₹ symbol. Use 4 decimal places for tiny amounts unless larger amounts warrant 2 decimals.
15. Add a short "Pricing notes" section listing models.dev lookup date, USD→INR rate and source/date, unmatched models, assumptions, Codex CLI parsing assumptions, and skipped/invalid session lines if any.

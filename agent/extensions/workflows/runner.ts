...
export interface RunAgentOptions {
  ...
  /** Explicit opt-in to parallel-safe discovery with a strict tool allowlist. */
  readOnly?: boolean;
  /** Test overrides. */
...
export function enforceReadOnlyWorkflowTools(
  ...
  const apply = () => session.setActiveToolsByName?.(allowed);
  apply();
...
export interface FirstResponseWatchdog {
  readonly promise: Promise<never>;
  observe(): void;
  dispose(): void;
}

/** 45-second first-provider-event deadline with stable diagnostics. */
export function createFirstResponseWatchdog(options: {
  provider?: string;
  model?: string;
  timeoutMs?: number;
}): FirstResponseWatchdog {
  let disposed = false;
  ...
  const promise = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `No first response within ${options.timeoutMs ?? FIRST_RESPONSE_TIMEOUT_MS} ms from ${options.provider ?? "unknown provider"}/${options.model ?? "unknown model"}`,
        ),
      );
    }, options.timeoutMs ?? FIRST_RESPONSE_TIMEOUT_MS);
    timer.unref?.();
  });
  const dispose = () => {
    if (disposed) return;
...
async function waitBounded(operation: Promise<unknown>, timeoutMs: number) {
  let timer: ReturnType<typeof setTimeout> | undefined;
  const timeout = new Promise<void>((resolve) => {
    timer = setTimeout(resolve, timeoutMs);
    timer.unref?.();
  });
  await Promise.race([operation.catch(() => {}), timeout]);
  if (timer) clearTimeout(timer);
}
...
export async function runAgent(
  ...
  let unsubscribeReadOnlyPolicy: (() => void) | undefined;
  try {
    customTools =
    ...
    if (options.readOnly) {
      unsubscribeReadOnlyPolicy = enforceReadOnlyWorkflowTools(
        session,
        options.schema !== undefined,
      );
    }
  ...
  const watchdog = createFirstResponseWatchdog({
    provider: childSession.model?.provider ?? options.model?.provider,
    model: childSession.model?.id ?? options.model?.id,
  ...
  const onAbort = () => {
    ...
    aborted = true;
    watchdog.dispose();
    rejectAbort(new Error("Agent was aborted"));
  ...
  try {
    ...
    // Once the terminal path wins the race, no late provider/tool event may
    // publish progress or revive watchdog state.
    ...
    if (abortPromise) {
      await waitBounded(
        abortPromise,
        options.abortTimeoutMs ?? AGENT_ABORT_TIMEOUT_MS,
      );
    }
    acceptingEvents = false;
    unsubscribe();
    unsubscribeToolTimeout?.();
    unsubscribeReadOnlyPolicy?.();
    // One terminal snapshot after listeners are guarded against late events.
    ...
    await shutdownAndDisposeChildSession(childSession, {
      timeoutMs: options.abortTimeoutMs ?? AGENT_ABORT_TIMEOUT_MS,
    });
  }

...
// --- Claude: paths + per-request occupancy, never the run aggregate ----------

test("Claude project-directory encoding handles Windows and POSIX paths", () => {
  assert.equal(
    claudeProjectDirectory("C:\\Projects\\pi-config"),
    "C--Projects-pi-config",
  );
  assert.equal(
    claudeProjectDirectory("/home/parth/pi.config"),
    "-home-parth-pi-config",
  );
});

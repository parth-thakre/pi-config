...
function workflowDetails(): WorkflowDetails {
  return {
    ...
    startedAt: 1,
    lifecycle: "prepared",
    repoRoot: "C:/repo with space",
    commonGitDir: "C:/repo with space/.git",
    baseRef: "origin/main",
    baseCommit: "a".repeat(40),
    branch: "pi/workflow/wf_fixture",
    worktreePath: "C:/Users/example/.pi/agent/wt/wf_fixture",
    phases: [],
    agents: [],
  };
}
...
test("live artifact persistence includes current agents and transcripts", () => {
  ...
  try {
    ...
    assert.equal(workflow.lifecycle, "prepared");
    assert.equal(workflow.repoRoot, details.repoRoot);
    assert.equal(workflow.commonGitDir, details.commonGitDir);
    assert.equal(workflow.baseRef, details.baseRef);
    assert.equal(workflow.baseCommit, details.baseCommit);
    assert.equal(workflow.branch, details.branch);
    assert.equal(workflow.worktreePath, details.worktreePath);
